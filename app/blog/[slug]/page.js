  ...
  return (
    <main className="min-h-screen bg-base-100">
      ...
      {loading ? (
        ...
        <article>
          <header className="bg-primary pt-40 pb-16 text-white lg:pt-48 lg:pb-24">
            <Container size="md">
              ...
              <h1 className="mt-5 text-5xl font-black leading-[0.98] tracking-[-0.06em] sm:text-7xl">{post.title}</h1>
              <p className="mt-7 max-w-2xl text-lg leading-relaxed text-white/65">{post.excerpt}</p>
            </Container>
          </header>
          <Container size="md" className="py-14 lg:py-20">
            {post.coverImage && <div className="relative mb-12 aspect-[16/9] overflow-hidden rounded-[2rem]"><Image src={post.coverImage} alt={post.title} fill priority className="object-cover" /></div>}
            <div className="whitespace-pre-wrap text-lg leading-8 text-primary/72">{post.content}</div>
