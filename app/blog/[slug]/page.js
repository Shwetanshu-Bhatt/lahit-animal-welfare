  ...
  return (
    <PublicSiteGate><main className="public-page min-h-screen bg-base-100">
      ...
      {loading ? (
        ...
        <section className="flex min-h-[80vh] items-center bg-primary pt-28 text-white"><Container><span className="eyebrow text-accent">Blog</span><h1 className="display-title mt-7 text-6xl uppercase">Story not found.</h1><Link href="/blog" className="mt-8 inline-flex items-center gap-2 font-bold text-accent"><ArrowLeft className="h-4 w-4" /> Back to blogs</Link></Container></section>
      ) : (
        <article>
          <header className="bg-primary pt-32 pb-12 text-white sm:pt-40 sm:pb-16 lg:pt-48 lg:pb-24">
            <Container size="md">
              <Link href="/blog" className="mb-10 inline-flex items-center gap-2 text-sm font-bold text-white/55 hover:text-accent"><ArrowLeft className="h-4 w-4" /> All blogs</Link>
